...
class Product(models.Model):
    ...
    def __str__(self):
        return self.display_name #will display actual name of product

    def save(self, *args, **kwargs):
        if self.name:
            stripe_product_r = stripe.Product.create(name=self.name)
            self.stripe_product_id = stripe_product_r.id # stripe product id based off of stripe product response
        if not self.stripe_price_id: #if there is no stripe price id, then we can create a stripe price
        ...
        if self.price != self.og_price:
            ...
            if self.stripe_product_id: # if there is a stripe product id, then we can update the price
                stripe_price_obj = stripe.Price.create(
                    product= self.stripe_product_id,
                    unit_amount=self.stripe_price,
                    currency="usd"
